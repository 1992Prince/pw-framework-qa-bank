...
### 1. What is auto-waiting in Playwright, and how does it work internally? or 
### What is Playwright's auto-waiting mechanism and how does it work?
...
- **Receives Events** — not blocked by an overlapping element, element isn't obscured by another element on top of it (e.g., a modal overlay)
...
- First Selenium don't have any auto-waits feature but playwright does.
- Selenim have Implicit or Explicit waits.
...
- Default timeout is **30 seconds** for most actions/[for assertions 5 secs] (configurable via `test.setTimeout()`, `expect.timeout`, or per-action `timeout` option)
