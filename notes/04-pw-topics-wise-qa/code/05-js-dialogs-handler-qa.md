...
MethodPurpose`dialog.message()`Reads the dialog's message text (synchronous — no `await` needed)

`dialog.accept()`Clicks OK (async — `await` required)

`dialog.accept('value')`Clicks OK and types a value (only meaningful for `prompt()`)

`dialog.dismiss()`Clicks Cancel (async — `await` required)

`dialog.defaultValue()`Returns the pre-filled default text already present in a `prompt()` box (not what you typed via `accept()`)

`dialog.type()`Returns the type of dialog — `'alert'`, `'confirm'`, `'prompt'`, or `'beforeunload'`
